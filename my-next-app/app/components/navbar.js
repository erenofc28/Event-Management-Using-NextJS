...
export default function Navbar() {
  ...
  const navItem =
    "font-satoshi !text-[#F5EA8A]  text-[12px] sm:text-[13px] lg:text-[14px] font-normal tracking-[0.22em] uppercase relative link";

  return (
    <>
      {/* DESKTOP NAV */}
      <nav className="hidden md:flex items-center gap-6 sm:gap-8 lg:gap-10 text-[#F5EA8A]">
        ...
        <button className={navItem} onClick={() => router.push("/aboutUs")}>
          ABOUT US
        </button>
        <button className={navItem} onClick={() => router.push("/contactUs")}>
          CONTACT
        </button>
      </nav>
      {/* MOBILE TOGGLE */}
      ...
      >
        ...
        {/* LINKS */}
        <div className="flex flex-col items-center justify-center h-full gap-8 text-[#F5EA8A]">
          <button
            onClick={() => {
              setOpen(false);
              router.push("/");
